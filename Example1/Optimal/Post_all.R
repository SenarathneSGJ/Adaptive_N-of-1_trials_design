Post_all <- function(data.mat,para,mu_pr,sigma_pr){
  post_dist=Laplace_approx(data.mat=data.mat,theta=as.numeric(para[1:5]),para2=as.numeric(para[-c(1:5)]),log_postf1=log_postF,mu_pr=mu_pr,sigma_pr=sigma_pr)
  mu_post1=post_dist$par
  hessian1=optimHess(data.mat=data.mat,par=mu_post1,fn=log_postF,mu_pr=mu_pr,sigma_pr=sigma_pr,para2=as.numeric(para[-c(1:5)]))
